...
class Hra:
    hrad = Lokace(["východ"], "Hrad", "Stojíš před okovanou branou gotického hradu, která je zřejmě jediným vchodem do pevnosti. Klíčová dírka je pokryta pavučinami, což vzbuzuje dojem, že je budova opuštěná.")
    les1 = Lokace(["západ", "východ"], "Les", "Jsi na lesní cestě, která se klikatí až za obzor, kde mizí v siluetě zapadajícího slunce. Ticho podvečerního lesa občas přeruší zpěv posledních ptáků.")
    les2 = Lokace(["jih", "západ", "východ"], "Lesní rozcestí", "Nacházíš se na lesním rozcestí.")
    les3 = Lokace(["západ", "východ"], "Les", "Jsi na lesní cestě, která se klikatí až za obzor, kde mizí v siluetě zapadajícího slunce. Ticho podvečerního lesa občas přeruší zpěv posledních ptáků.")
    ...
    les4 = Lokace(["sever", "východ"], "Les", "Jsi na lesní cestě, která se klikatí až za obzor, kde mizí v siluetě zapadajícího slunce. Ticho podvečerního lesa občas přeruší zpěv posledních ptáků.")
    ...
    def interakce(self):
        smer = " "
        while smer != "":
            smer = input("Zadej příkaz (sever | východ | jih | západ) nebo ENTER pro ukončení: ")
            print(150 * '=')
            if smer not in ["sever", "jih", "západ", "východ", ""]:
                print("Můj vstupní slovník neobsahuje tento příkaz.")
            if self.pozice.kontrola(smer):
                print(self.pohyb(smer))
            elif smer == "":
                print("Pohyb ukončen")
            else:
